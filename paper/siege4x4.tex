...
unknown game or to introduce new general safety-game theory. This paper is a
verifiable computational case study: it contributes an explicit certificate and
verification for this rule set, not a new proof method. The rules
(Section~\ref{sec:game}) are unusual in three respects
...
\centering
\begin{tabular}{ccccc}
\input{fig_cap_ok1} & \input{fig_cap_ok2} & \input{fig_cap_no1} & \input{fig_cap_no2} & \input{fig_cap_no3}\\
...
\centering
\begin{tabular}{ccc}
\input{fig_region} & \qquad & \input{fig_fortress}\\
...
\begin{quote}\input{example_entry}\end{quote}
